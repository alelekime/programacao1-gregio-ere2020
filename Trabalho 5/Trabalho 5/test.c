/*
  ...
  Em 18/10/2019.
  Foi feito para testar exaustivamente a biblioteca que implementa
  o Tipo Abstrado de Dados lista, conforme os arquivos lista.c e lista.h
*/
...
int testa_remocoes(t_lista *l, t_lista *o, t_lista *c)
{
    ...
    while (!lista_vazia(l))
        if (!remove_primeiro_lista(&item, l))
            return 0;
    printf("l: ");
    imprime_lista(l);
    printf("\n");
    ...
    if (remove_item_lista(0, &item, c))
        printf("removeu %d\n", 0);
    if (remove_item_lista(8, &item, c))
        printf("removeu %d\n", 8);
    if (remove_item_lista(7, &item, c))
        printf("removeu %d\n", 7);
    if (remove_item_lista(12, &item, c))
        printf("removeu %d\n", 12);
    printf("c: ");
    imprime_lista(c);
    return 1;
...
int main()
{
    ...
    if (!testa_copia(&o, &c))
        return 1;
    if (!testa_concatencao(&l, &m))
        return 1;
    if (!testa_remocoes(&l, &o, &c))
        return 1;
    testa_busca(&c);
